...
/* ===== V4.5 Final UI Fix: Editor Tools Visible + Safe Fallback ===== */
(function(){
  function hasFn(name){ return typeof window[name] === 'function'; }
  function getClipsSafeV45(){
    if(typeof timelineClips === 'undefined' || !Array.isArray(timelineClips)) window.timelineClips=[];
    return timelineClips;
  }
  function selectedClipSafeV45(){
    const clips=getClipsSafeV45();
    if(!selectedClipId && window.selectedClipIdsV2 && window.selectedClipIdsV2[0]) selectedClipId=window.selectedClipIdsV2[0];
    return clips.find(c=>c.id===selectedClipId) || null;
  }
  function setEditorToolsStatusV45(){
    const st=document.getElementById('editorToolsStatusV45');
    if(!st)return;
    const c=selectedClipSafeV45();
    st.textContent=c ? ('Clip: '+(c.name||c.note||c.id)) : 'Pilih clip untuk edit cepat';
  }
  window.toggleEditorToolsV45=function(force){
    const box=document.getElementById('editorToolsPanelV45');
    if(!box)return;
    const show=typeof force==='boolean'?force:!box.classList.contains('show');
    box.classList.toggle('show',show);
    setEditorToolsStatusV45();
  };
  window.openEditorToolsV45=function(){ window.toggleEditorToolsV45(true); };
  window.closeEditorToolsV45=function(){ window.toggleEditorToolsV45(false); };

  if(!hasFn('pushUndoV2')){
    window.timelineUndoStackV45=[];
    window.timelineRedoStackV45=[];
    window.pushUndoV2=function(){
      try{
        window.timelineUndoStackV45.push(JSON.stringify(getClipsSafeV45()));
        if(window.timelineUndoStackV45.length>30)window.timelineUndoStackV45.shift();
        window.timelineRedoStackV45=[];
      }catch(e){console.warn(e);}
    };
  }
  if(!hasFn('undoTimelineV2')){
    window.undoTimelineV2=function(){
      if(!window.timelineUndoStackV45 || !window.timelineUndoStackV45.length){alert('Belum ada Undo.');return;}
      try{
        window.timelineRedoStackV45.push(JSON.stringify(getClipsSafeV45()));
        window.timelineClips=timelineClips=JSON.parse(window.timelineUndoStackV45.pop());
        selectedClipId=null;
        if(typeof updateRecordList==='function')updateRecordList();
        if(typeof updateImportList==='function')updateImportList();
        if(typeof renderTimeline==='function')renderTimeline();
  ...
  if(!hasFn('redoTimelineV2')){
    window.redoTimelineV2=function(){
      if(!window.timelineRedoStackV45 || !window.timelineRedoStackV45.length){alert('Belum ada Redo.');return;}
      try{
        window.timelineUndoStackV45.push(JSON.stringify(getClipsSafeV45()));
        window.timelineClips=timelineClips=JSON.parse(window.timelineRedoStackV45.pop());
        selectedClipId=null;
        if(typeof updateRecordList==='function')updateRecordList();
        if(typeof updateImportList==='function')updateImportList();
        if(typeof renderTimeline==='function')renderTimeline();
  ...
  if(!hasFn('duplicateSelectedClip')){
    window.duplicateSelectedClip=function(){
      const c=selectedClipSafeV45();
      if(!c){alert('Pilih clip dulu.');return;}
      if(typeof pushUndoV2==='function')pushUndoV2();
      const d=JSON.parse(JSON.stringify(c));
      d.id=(typeof uid==='function')?uid():('clip'+Date.now());
      d.name=(d.name||'Clip')+' Copy';
      d.startTime=typeof playheadTime==='number'?playheadTime:((Number(c.startTime)||0)+(Number(c.duration)||1));
      getClipsSafeV45().push(d);
      selectedClipId=d.id;
      if(typeof setSelectionV2==='function')setSelectionV2([d.id]);
      if(typeof updateRecordList==='function')updateRecordList();
      if(typeof updateImportList==='function')updateImportList();
      if(typeof renderTimeline==='function')renderTimeline();
      window.toggleEditorToolsV45(true);
    };
  }
  if(!hasFn('scrollTimelineTop')){
    window.scrollTimelineTop=function(){
      const p=document.getElementById('timelinePanel');
      const s=document.getElementById('timelineScroll');
      try{ if(p)p.scrollTop=0; if(s)s.scrollTop=0; }catch(e){}
    };
  }
  if(!hasFn('scrollTimelineBottom')){
    window.scrollTimelineBottom=function(){
      const p=document.getElementById('timelinePanel');
      const s=document.getElementById('timelineScroll');
      try{ if(p)p.scrollTop=p.scrollHeight; if(s)s.scrollTop=s.scrollHeight; }catch(e){}
    };
  }

  const oldRenderTimelineV45 = window.renderTimeline;
  if(typeof oldRenderTimelineV45 === 'function' && !oldRenderTimelineV45.__editorToolsPatchV45){
    window.renderTimeline=function(){
      const r=oldRenderTimelineV45.apply(this,arguments);
      setEditorToolsStatusV45();
      return r;
    };
    window.renderTimeline.__editorToolsPatchV45=true;
  }
  document.addEventListener('DOMContentLoaded',function(){ setEditorToolsStatusV45(); });
})();
